...
int main() {
	...
	for (index; index < 99;) {
		if ((((float)(bad_count + normal_count) / (float)(index + 9)) * 100) >= 23) {
			compare1Init();
		}
		else if (((float)bad_count / (float)(bad_count + normal_count) * 100) <= 66 && ((float)bad_count / (float)(bad_count + normal_count) * 100) >= 34) {
			compare1Init();
		}
		else {
			compare3Init();
...
void confirmCompare1() {
	...
	if (percentage <= 25 && too_fast_convert_3compare == FALSE) {
		...
		if (double_unsure_count != 0) {
			if (sum_sure_count <= 66 && percentage >= 57) {
			...
			else if (sum_sure_count <= 66 && percentage <= 43) {
				for (j = 0; j < double_unsure_count; j = j + 3) {
			...
			else if (percentage >= 60) {		// 불량 동전이 많을 경우
				b[0] = bad_coin[0];
				c[0] = bad_coin[1];
