...
int main() {
	...
	for (; index < 99; ) {
		// 확정된 동전 중 불량 동전 비율이 33%이상 66% 이하이고 
		// 탐색한 인덱스 중 확정된 동전의 비율이 65% 이상
		if ((((float)bad_count / (float)(bad_count + normal_count) * 100) <= 66 && ((float)bad_count / (float)(bad_count + normal_count) * 100) >= 33) && (((float)(bad_count + normal_count) / (float)(index)) * 100) >= 65)
			compare1Init();
		else {
			compare3Init();
		}
...
void confirmCompare1() {
	...
	// 불량 동전 비율이 75% 이상일 경우 뒤에 unsure_1compare에 해당되는 내용은 모두 불량 동전으로 바꿈
		...
		if (double_unsure_count != 0) {
			if (sum_sure_count <= 60 && percentage >= 53) {
			...
			else if (sum_sure_count <= 60 && percentage <= 47) {
				for (j = 0; j < double_unsure_count; j = j + 3) {
